...
func (s *userStore) CreateRefreshToken(ctx context.Context, refreshTokenData types.UserRefreshToken) (int, error) {
	result := s.db.GetDb().WithContext(ctx).Model(&types.UserRefreshToken{}).Create(&refreshTokenData)
...
func (s *userStore) RevokedRefreshToken(ctx context.Context, userid int, refreshToken string) error {
	result := s.db.GetDb().WithContext(ctx).Model(&types.UserRefreshToken{}).Where("user_id = ? and token = ?", userid, refreshToken).Update("revoked", true)
	return result.Error
...
func (s *userStore) GetRefreshTokenData(ctx context.Context, userid int, refreshToken string) (types.UserRefreshToken, error) {
	...
	result := s.db.GetDb().WithContext(ctx).Model(&types.UserRefreshToken{}).Select("id", "expires_at", "revoked").Where("user_id = ? and token = ?", userid, refreshToken).First(&tokenData)
