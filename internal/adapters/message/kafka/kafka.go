...
func New(broker []string, conf config.Kafka) (port.Messager, error) {
...
func (k *kafkaMessager) PublishActivationEmail(toAddress string, subject string, content string) error {
...
func (k *kafkaMessager) PublishPasswordResetEmail(toAddress string, subject string, content string) error {
	payload := map[string]string{
		"email":   toAddress,
		"subject": subject,
		"content": content,
...
func (k *kafkaMessager) publish(topic string, key string, payload map[string]string) error {
